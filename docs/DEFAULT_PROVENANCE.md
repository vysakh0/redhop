...
| `bm25_field_weights` | **uniform (1.0 / 1.0 / 1.0)** | 🟢 [`CATALOG_REGIME.md`](findings/CATALOG_REGIME.md) + [`BM25_SOURCE_FIELD.md`](findings/BM25_SOURCE_FIELD.md) | Per-field BM25 boosts for `text` / `source` / `heading`. Equal weight is the measured default for prose; non-uniform is a domain lever for near-duplicate / title-heavy corpora. A `1.0` weight is skipped before Tantivy, so uniform is bit-for-bit the prior behavior. The set-coverage *lift* was an exact **null** on the synthetic catalog (a boost on a field the near-duplicates share is inert), so the default stays uniform and the guidance is "sweep on your own eval". |
