...
| [SEMANTIC_MISMATCH](SEMANTIC_MISMATCH.md) | **Confirmed: lexical-first + local rerank** (controlled + natural, Tier 1+3) | Dense helps semantic-heavy queries (+0.16 F1); no cheap escalation *trigger* exists (overlap + BM25 margin/entropy both null). Resolution: BM25-prune → *local* dense rerank of the candidate pool recovers 96% of global dense's gains at bounded cost, no ANN — conditional on BM25 candidate recall@K (0.94 here) | `cargo run -p redhop-examples --example semantic_local_rerank --features onnx --release` |
