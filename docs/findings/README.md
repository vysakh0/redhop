...
| [LOCAL_RERANK](LOCAL_RERANK.md) | **Confirmed** (global HotpotQA, Tier 1+3, n=400) | BM25 prunes → dense reranks only the candidate pool: matches *global* dense on recall (0.80) and answers (0.54 F1), beats hybrid, recovers ~96% of dense's gains — at K-candidate cost, **no ANN, no escalation trigger**. Cap: needs BM25 candidate recall@K (0.94 here) | `cargo run -p redhop-examples --example semantic_local_rerank --features onnx --release` + `python python/eval/score_local_rerank.py` |
