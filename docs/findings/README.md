...
| [CUAD_CLAUSE_EXPANSION](CUAD_CLAUSE_EXPANSION.md) | **Confirmed** (n=300, +2.7 pts over template-stripped baseline → 90.3%) | Ships `redhop::expand_query_terms` as the additive symmetric to `drop_template_terms`. Hand-curated CUAD clause-name → synonyms dict (34 keys, 121 syns) added to the template-stripped query lifts ≥0.8 retention 87.7% → 90.3%, beating LlamaIndex's 86% by 4 points. Control arm (raw + expanded) confirms expansion helps independently of stripping. **Mechanism** is the opposite of unweighted PRF (CUAD_PRF_NULL): static workload-curated **high-IDF** terms instead of corpus-pervasive **low-IDF** terms. Same workload-specific discipline as drop_template_terms — library ships the mechanism, user supplies the dict. | `cargo run -p redhop-examples --example cuad_clause_expansion --release` |
