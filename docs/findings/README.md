...
| [SEMANTIC_MISMATCH](SEMANTIC_MISMATCH.md) | **Confirmed value / falsified trigger** (controlled + natural, Tier 1+3) | Dense helps semantic-heavy queries (+0.16 F1, ~0 on lexical) — value real & observable. But no cheap deterministic escalation *trigger* works: query↔hit overlap and BM25 margin/entropy both fail to separate the regimes; conditional escalation = linear interpolation, no selective gain | `cargo run -p redhop-examples --example semantic_natural --features onnx --release` |
