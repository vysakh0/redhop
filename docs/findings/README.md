...
| [SEMANTIC_MISMATCH](SEMANTIC_MISMATCH.md) | **Confirmed (conditional)** (controlled, BM25/dense/hybrid) | Maps the lexical↔semantic boundary: BM25 perfect where lexis aligns, fails completely under paraphrase/synonymy (R@1 0%); dense recovers (R@1 ~68%); naive RRF hybrid is *worse* than dense in mismatch. Motivates lexical-first + conditional dense escalation | `cargo run -p redhop-examples --example semantic_mismatch --features onnx --release` |
