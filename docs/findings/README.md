...
| [LOCAL_RERANK](LOCAL_RERANK.md) | **Confirmed, then superseded as a tier** (global HotpotQA, Tier 1+3, n=400) | BM25 prunes → dense reranks only the pool: matched *global* dense on recall (0.80) / answers (0.54 F1). But global dense ≥ it on recall at ~equal cost (see GLOBAL_DENSE), so the rerank *tier* was dropped — kept here as the evidence that justified local-first, now folded into `dense`. | `cargo run -p redhop-examples --example semantic_local_rerank --features onnx --release` + `python python/eval/score_local_rerank.py` |
...
| RM3 / pseudo-relevance feedback lifts lexical recall | **Falsified (this workload)** | Monotonically harmful (semantic R@3 0.49→0.41); λ=1 (no feedback) is optimal. Low first-pass precision (R@1≈0.31) → feedback built on distractors | Don't bolt PRF onto BM25 here; another instance of the second-hop/relevance-feedback tax |
