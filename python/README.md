...
| `vocab.enrich(chunk_text)` | Chunk-side mirror, **shipped on mechanism reasoning with asymmetric evidence** — measured negative on CUAD (−2.0pt), positive case on short-opaque coded units (schemas, error codes, API symbols) mechanism-predicted but unmeasured. A/B on your own corpus before adopting | [VOCABULARY_ENRICH](https://github.com/vysakh0/redhop/blob/main/docs/findings/VOCABULARY_ENRICH.md) + [CUAD_ENRICH_DEFINITIONS_NULL](https://github.com/vysakh0/redhop/blob/main/docs/findings/CUAD_ENRICH_DEFINITIONS_NULL.md) |
