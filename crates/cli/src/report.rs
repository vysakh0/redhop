//! `neorag report` — render a benchmark/compare JSON artifact to markdown/HTML.
//! Reads artifacts produced by `neorag compare --json` or the
...
fn flatten(v: &Value) -> anyhow::Result<(String, Vec<String>, Vec<Vec<String>>, Vec<String>)> {
    ...
    let title = v
        ...
        .unwrap_or_else(|| "NeoRAG report".into());
...
fn render_markdown(title: &str, headers: &[String], rows: &[Vec<String>], meta: &[String]) -> String {
    ...
    s.push_str("\n_Generated by `neorag report`. Evidence: docs/findings/._\n");
    s
}
...
<p style=\"color:#666;font-size:13px\">Generated by <code>neorag report</code>. \
