...
fn terms(text: &str) -> HashSet<String> {
    text.unicode_words().map(|w| w.to_lowercase()).filter(|w| w.chars().count() > 1).collect()
}
...
struct Lcg(u64);
impl Lcg {
    fn new(s: u64) -> Self { Self(s.wrapping_mul(0x9E3779B97F4A7C15).wrapping_add(1)) }
...
fn as_results(chunks: &[Chunk]) -> Vec<RetrievalResult> {
    chunks.iter().map(|c| RetrievalResult {
        chunk: c.clone(),
...
fn mean(xs: &[f32]) -> f32 { if xs.is_empty() { 0.0 } else { xs.iter().sum::<f32>() / xs.len() as f32 } }
...
fn main() -> anyhow::Result<()> {
    ...
    for lq in &corpus.queries {
        if lq.gold_chunk_ids.len() < 2 { continue; }
        let q_terms = terms(&lq.text);
        let mut gold: Vec<(ChunkId, Chunk, f32)> = lq.gold_chunk_ids.iter()
            .filter_map(|id| by_id.get(id).map(|c| (id.clone(), c.clone())))
            .map(|(id, c)| { let g = grounding(&q_terms, &terms(&c.text)); (id, c, g) })
            .collect();
        if gold.len() < 2 { continue; }
        ...
        let mut pool: Vec<&Chunk> = chunks.iter().filter(|c| !gold_docs.contains(&c.source)).collect();
        for i in (1..pool.len()).rev() { let j = (rng.next() as usize) % (i + 1); pool.swap(i, j); }
        let mut all: Vec<Chunk> =
            gold_chunks.iter().cloned().chain(pool.iter().take(N_DISTRACTORS).map(|c| (*c).clone())).collect();
    ...
    for case in &cases {
        let pop = if case.is_gap { "multihop_gap" } else { "shallow_nogap" };
        for (sname, strat) in STRATEGIES {
            for budget in BUDGETS {
                ...
                cell.gold_ret.push(gold_in as f32 / case.gold_ids.len() as f32);
                cell.second_ret.push(if kept.contains(&&case.second_hop_id) { 1.0 } else { 0.0 });
                cell.density.push(ctx.report.economics.evidence_density);
                cell.out_distractor.push(ctx.report.economics.distractor_ratio);
    ...
    json.push_str("{\n");
    write!(json, "  \"benchmark\": \"context_strategies\",\n")?;
    write!(json, "  \"metadata\": {{\n")?;
    write!(json, "    \"generated_unix\": {ts},\n")?;
    write!(json, "    \"dataset\": \"hotpotqa_dev_distractor\",\n")?;
    write!(json, "    \"sample_size\": {SAMPLE_SIZE},\n")?;
    write!(json, "    \"n_multihop_gap\": {n_gap},\n")?;
    write!(json, "    \"n_shallow_nogap\": {n_nogap},\n")?;
    write!(json, "    \"n_distractors\": {N_DISTRACTORS},\n")?;
    write!(json, "    \"distractor_min_grounding\": {DISTRACTOR_MIN_GROUNDING},\n")?;
    write!(json, "    \"link_min_jaccard\": {LINK_MIN_JACCARD},\n")?;
    write!(json, "    \"budgets\": {BUDGETS:?},\n")?;
    write!(json, "    \"hermetic\": true\n")?;
    write!(json, "  }},\n")?;
    write!(json, "  \"results\": [\n")?;
    ...
    for pop in ["multihop_gap", "shallow_nogap"] {
        ...
        for (sname, _) in STRATEGIES {
            for budget in BUDGETS {
                let Some(cell) = cells.get(&(pop, sname, budget)) else { continue };
                ...
                write!(json, "\"gold_ret\":{g:.4},\"gold_ret_ci\":[{glo:.4},{ghi:.4}],")?;
                write!(json, "\"second_hop_ret\":{s:.4},\"second_hop_ret_ci\":[{slo:.4},{shi:.4}],")?;
                write!(json, "\"evidence_density\":{dens:.4},\"output_distractor_ratio\":{od:.4},")?;
